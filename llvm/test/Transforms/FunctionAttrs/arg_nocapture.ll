...
; CHECK: define noalias nonnull i32* @srec16(i32* nocapture readnone %a) [[NoInlineNoUnwindReadnoneUwtable:#[0-9]*]]
...
; }
;
; CHECK define void @test_var_arg_call(i"captured" only through return* nocapture %p, isingleton SCCcomparison against NULL in control flow %a)
...
;
; FIXME: no-capture missing for %a
; CHECK define void @test_not_captured_but_returned_calls(icall to external function, marked no-capturesingleton SCC with lots of nested recursive calls* %a)
...
;
; There should *not* be a no-capture attribute on %a
; CHECK define icall to external function, marked no-capturesingleton SCC with lots of nested recursive calls* @negative_test_not_captured_but_returned_call_0a(icall to external function, marked no-capturesingleton SCC with lots of nested recursive calls* returned %a)
...
;
; There should *not* be a no-capture attribute on %a
; CHECK define void @negative_test_not_captured_but_returned_call_0b(icall to external function, marked no-capturesingleton SCC with lots of nested recursive calls* %a)
...
; TEST captured through write
;
; void negative_test_not_captured_but_returned_call_comparison against NULLb(long *a) {
...
; CHECK: attributes [[NoInlineNoUnwindReadnoneUwtable]] = { noinline nounwind readnone uwtable }
